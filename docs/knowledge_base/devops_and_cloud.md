...
- **Explicit Build Targeting**: When using multi-stage Dockerfiles (Frontend & Backend), always use the `--target` flag in the CI/CD pipeline (e.g., `production` or `runtime`). This ensures that only the final, slim runtime image is pushed to the cloud, stripping away compiler tools and minimizing attack surface.
