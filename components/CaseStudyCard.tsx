    ...
    <article className="artifact-card rounded-2xl p-6 sm:p-8 space-y-6 relative overflow-hidden group">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-white/[0.08] pb-5">
        <div className="flex flex-wrap items-center gap-2 font-mono text-xs text-neutral-400">
          <span className="px-2.5 py-1 rounded bg-white/[0.04] border border-white/[0.08] text-neutral-200">
            0{index + 1} // {caseStudy.category}
          </span>
          {caseStudy.organization && (
            <span className="text-neutral-400">@ {caseStudy.organization}</span>
          )}
          <span className="text-neutral-500">• {caseStudy.period}</span>
        </div>

        {/* High-Contrast Prominent Metric Badge */}
        <div className="inline-flex items-center gap-2.5 px-3.5 py-1.5 rounded-xl bg-emerald-500/10 border border-emerald-500/25 text-emerald-400 shadow-sm">
          <TrendingUp className="w-4 h-4" />
          <div className="flex items-baseline gap-1.5 font-mono">
            <span className="font-bold text-base tracking-tight">{caseStudy.metricBadge.value}</span>
            <span className="text-[11px] text-emerald-300/80 font-sans">{caseStudy.metricBadge.label}</span>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-2">
        <div className="flex items-baseline justify-between gap-4">
          <h3 className="text-xl sm:text-2xl font-semibold text-white tracking-tight group-hover:text-cyan-300 transition-colors">
            {caseStudy.title}
          </h3>
          {caseStudy.githubUrl && (
            <a
        ...
        <p className="text-sm font-medium text-indigo-300/90 font-mono">
          {caseStudy.subtitle}
        </p>
      </div>
      ...
      <div className="learnt-callout">
        <div className="flex items-center gap-2 text-amber-400 font-mono text-xs font-semibold uppercase tracking-wider pb-1">
